...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', (e) => {
        const imagenesValidas = [imgPrincipal, imgsecun1, imgsecun2].every(validarImagen);
        if (!imagenesValidas) {
            e.preventDefault();
            return;
        }
        ...
        const camposImagenes = [
            { input: imgPrincipal, nombre: 'Imagen principal' },
            { input: imgsecun1, nombre: 'Imagen 2' },
            { input: imgsecun2, nombre: 'Imagen 3' }
        ];

        let imagenesSubidas = true;
        camposImagenes.forEach(({ input, nombre }) => {
            if (!input.files || input.files.length === 0) {
                alert(`Debes subir ${nombre}.`);
                imagenesSubidas = false;
            }
        });
